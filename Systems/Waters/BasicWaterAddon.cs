...
namespace LunarVeil.Systems.Waters
{
    internal class BasicWaterAddon : WaterAddon
    {
        ...
        private static void RenderFront(SpriteBatch spriteBatch)
        {
            ...
            Texture2D tex2 = ModContent.Request<Texture2D>("LunarVeil/Assets/NoiseTextures/Water2", ReLogic.Content.AssetRequestMode.ImmediateLoad).Value;
            ...
            tex2 = ModContent.Request<Texture2D>("LunarVeil/Assets/NoiseTextures/WaterGradient", ReLogic.Content.AssetRequestMode.ImmediateLoad).Value;
            for (int i = -tex2.Width; i <= Main.screenWidth + tex2.Width; i += tex2.Width)
            {
                for (int j = -tex2.Height; j <= Main.screenHeight + tex2.Height; j += tex2.Height)
                {
